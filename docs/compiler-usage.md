...
```text
mesor SOURCE [SOURCE ...] [-o EXECUTABLE] [--emit-c FILE] [--cc COMMAND] [--run]
```

`mesor` is the shell wrapper around `mesor.py`. Each input is UTF-8
...
| Option | Meaning |
| --- | --- |
| `SOURCE [SOURCE ...]` | One or more Mesor files. |
...
```sh
./mesor examples/scaled.neo --run
./mesor program.neo --cc gcc --emit-c program.c -o program
./mesor units.neo library.neo app.neo -o app
```
...
```text
mesor: error: path/to/file.neo:12: explanation
```
